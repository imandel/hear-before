...
# hex_centers = pd.read_csv('hex_centers.csv',delim_whitespace=True)
# hex_pts = list(zip(hex_centers['lat'].tolist(), hex_centers['lng'].tolist()))

# def find_closest_pt(available_pts, lat, lng):
#     min_dist = None
#     min_pt = None
#     for pt in available_pts:
#         dist = math.sqrt( (pt[0] - lat)**2 + (pt[1] - lng)**2 )
#         if min_pt is None or min_dist > dist:
#             min_pt = pt
#             min_dist = dist
#     return min_pt

# original_mappings = pd.read_csv('content/synth_text_mappings.csv')

# available_pts = set(hex_pts)
# pt_mapping = {}
# source_pt_list = list(zip(original_mappings['lat'].tolist(), original_mappings['lng'].tolist()))
# for lat, lng in source_pt_list:
#     closest_pt = find_closest_pt(available_pts, lat, lng)
#     available_pts.remove(closest_pt)
#     pt_mapping[(lat, lng)] = closest_pt


# original_mappings['new_pt'] = original_mappings.apply(lambda x: pt_mapping[(x['lat'], x['lng'])], axis = 1)
# original_mappings['new_lat'] = original_mappings['new_pt'].apply(lambda x: x[0])
# original_mappings['new_lng'] = original_mappings['new_pt'].apply(lambda x: x[1])

# original_mappings.to_csv('content/adjust_synth_mappings.csv')
...
df = pd.read_csv('content/adjusted_synth_mappings_only_lat_lng.csv').to_csv('content/adjusted_synth_mappings_only_lat_lng.csv', index=False)

# lat_lngs = list(zip(df['lat'].tolist(),df['lng'].tolist()))
...
# with open('content/adjusted_synth_mapping.json', 'w+') as outfile:
#     json.dump(all_points_geo, outfile)